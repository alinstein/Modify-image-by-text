...
class STAGE1_G(nn.Module):
    ...
    def forward(self, text_embedding, noise):
        ...
        #print(noise.shape,c_code.shape,"dsdfd")
...
class STAGE2_G(nn.Module):
    ...
    def _make_layer(self, block, channel_num):
        layers = []
        #number of Residual is 4( changing into 2 )
        Residual_layer =2
    ...
    def define_module(self):
        ...
        self.ca_net = CA_NET()
        # --> 4ngf x 16 x 16
        #Encoder for Stage one generated images
        self.encoder = nn.Sequential(
            #3 Channel to ngf channels
            conv3x3(3, ngf),
            nn.ReLU(True),
    ...
    def forward(self, text_embedding, noise):
        #Generating image from perviously trained stage one model
        #print("After",text_embedding.shape, noise.shape)
        _, stage1_img, _, _ = self.STAGE1_G(text_embedding, noise)
        #detaching to remove gradient
        stage1_img = stage1_img.detach()
        #Passing on the generated image to encoder to extract the features
...
class STAGE2_D(nn.Module):
    ...
    def define_module(self):
        ndf, nef = self.df_dim, self.ef_dim
        self.encode_img = nn.Sequential(
            nn.Conv2d(3, ndf, 4, 2, 1, bias=False),  # (128 * 128) 64 x 64 * ndf)
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(ndf, ndf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(ndf * 2),
